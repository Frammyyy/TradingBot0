...
import pandas as pd
import os
from STRATS import strat, dynamic_tp_sl
from configs import API, TIME, PAIR_CONFIG
import oandapyV20
...
def get_price_data(symbol, lookback=LOOPBACK, granularity="M5"):
    ...
    :return: DataFrame with datetime index and 'Close' price column.
    """
    try:
        params = {
            "count": lookback,
        ...
        # Calculate indicators using your strat module
...
def main_trade_loop():
    # Initialize per-symbol state dict with trailing TP state if enabled
    symbol_state = {
        sym: {
            'in_position': False,
            'position_type': None,
            'entry_price': None,
            'trailing_state': dynamic_tp_sl.initialize_trailing_state(cfg) if cfg.get('trail_tp_enabled') else None
        }
    ...
    while True:
        ...
        for symbol, cfg in PAIR_CONFIG.items():
            ...
            # Sync state if position exists on broker but not tracked locally
            if has_position and not state['in_position']:
                state.update({'in_position': True, 'position_type': 'long' if pos_units > 0 else 'short', 'entry_price': pos_price})
            elif not has_position:
                state.update({'in_position': False, 'position_type': None, 'entry_price': None})

            if not state['in_position']:
                # Reset trailing TP state on new position
                state['trailing_state'] = dynamic_tp_sl.initialize_trailing_state(cfg) if cfg.get('trail_tp_enabled') else None
                ...
                # Check trailing TP/SL if enabled and trailing state exists
                if cfg.get('trail_tp_enabled') and state['trailing_state']:
                    triggered, trail_reason = dynamic_tp_sl.update_trailing_tp(
                        state['trailing_state'],
                        close_price,
                        entry_price,
                        state['position_type'],
                        decay=cfg.get('trail_tp_decay', 0.5)
                    )
                    if triggered:
                        exit_trade, reason = True, trail_reason

                sl = cfg['sl_pct']
                tp = cfg['tp_pct']

                # Check normal stop loss and take profit exits if not already exiting
                if not exit_trade:
                ...
                if exit_trade:
                    ...
                    # Reset local state after exit
                    state.update({'in_position': False, 'position_type': None, 'entry_price': None, 'trailing_state': None})
